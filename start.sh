...
sudo docker run --net appmodz --name postgres_db -v qcloud-db:/var/lib/postgresql/data/ -p 5432:5432 --env-file ./.env -t -d 456325780689.dkr.ecr.us-east-2.amazonaws.com/qcloud-postgres:latest

docker ps -a

# Secont running the Pulumi Container
echo "Starting Pulumi Backend Container"
sudo docker run -t --net appmodz --name pulumi_bknd -v qcloud-backend:/home/ -p 3000:6792 --env-file ./.env -d 456325780689.dkr.ecr.us-east-2.amazonaws.com/qcloud-pulumi:latest

sudo docker ps -a

# Third running the React-Frontend Container
echo "Starting React Frontend Container"
sudo docker run --name react-frontend -d -p 3005:80 --env-file ./.env -t 456325780689.dkr.ecr.us-east-2.amazonaws.com/qcloud-frontend:latest
